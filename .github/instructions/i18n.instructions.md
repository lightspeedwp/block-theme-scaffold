...
You are an internationalization specialist. Follow our WordPress block theme i18n patterns to make every user-facing string translatable and consistent. Avoid string concatenation, mismatched text domains, or unescaped output in translations.
...
2. **Consistent Text Domain**: Use `{{theme_slug}}` as the text domain throughout
...
Use the sections below for text domain requirements, PHP and JavaScript translation patterns, translator comments, placeholders, build tooling, and testing workflows specific to this scaffold.

## Text Domain

**Theme Text Domain**: `{{theme_slug}}`
...
```php
// ✅ Correct
$title = __('Welcome to our theme', '{{theme_slug}}');
...
```php
// ✅ Correct
<h1><?php _e('Site Title', '{{theme_slug}}'); ?></h1>

// ❌ Incorrect
<h1><?php echo __('Site Title', '{{theme_slug}}'); ?></h1> // Use _e() instead
```
...
$text = esc_html__('Hello World', '{{theme_slug}}');

// ✅ Correct - echoing escaped
<p><?php esc_html_e('Welcome Message', '{{theme_slug}}'); ?></p>

// ❌ Incorrect - double escaping
echo esc_html(esc_html__('Text', '{{theme_slug}}')); // Don't do this!
```
...
<input
    type="text"
    placeholder="<?php echo esc_attr__('Enter your name', '{{theme_slug}}'); ?>"
/>

// ✅ Correct - direct output
<input
    type="text"
    aria-label="<?php esc_attr_e('Search', '{{theme_slug}}'); ?>"
/>
```
...
$label = _x('Post', 'noun: blog post', '{{theme_slug}}');

// ✅ Correct - "Post" as in verb
$button = _x('Post', 'verb: publish action', '{{theme_slug}}');

// Context helps translators understand the meaning
<button><?php echo esc_html(_x('Read', 'verb: read article', '{{theme_slug}}')); ?></button>
```
...
const title = __('Block Settings', '{{theme_slug}}');

// ✅ Correct - with context
const label = _x('Media', 'block category', '{{theme_slug}}');
...
function MyComponent() {
    return (
        <div>
            <h2>{__('Component Title', '{{theme_slug}}')}</h2>
            <p>{__('Description text here', '{{theme_slug}}')}</p>
        </div>
    );
}
```
...
const message = sprintf(
    __('You have %s items in your cart', '{{theme_slug}}'),
    count
);
```
...
$meta = sprintf(
    __('Published on %1$s by %2$s', '{{theme_slug}}'),
    get_the_date(),
    get_the_author()
);
...
// ❌ Incorrect - concatenation
$text = __('Published on', '{{theme_slug}}') . ' ' . get_the_date();
```
...
**Generates**: `languages/{{theme_slug}}.pot`
...
**Generates**: `languages/{{theme_slug}}-{locale}-{hash}.json`
...
```
languages/
├── {{theme_slug}}.pot              # Template file (generated)
├── {{theme_slug}}-es_ES.po         # Spanish translation (manual)
├── {{theme_slug}}-es_ES.mo         # Spanish compiled (generated)
├── {{theme_slug}}-es_ES-{hash}.json # Spanish JS (generated)
├── {{theme_slug}}-fr_FR.po         # French translation (manual)
├── {{theme_slug}}-fr_FR.mo         # French compiled (generated)
└── {{theme_slug}}-fr_FR-{hash}.json # French JS (generated)
```
...
<div class="entry-meta">
    ...
    printf(
        ...
        esc_html__('Posted on %1$s by %2$s', '{{theme_slug}}'),
...
if ($has_posts) {
    echo esc_html__('Results found', '{{theme_slug}}');
} else {
    echo esc_html__('No results found', '{{theme_slug}}');
}

// ❌ Incorrect - combining strings
echo esc_html__($has_posts ? 'Results' : 'No results', '{{theme_slug}}') . ' ' . __('found', '{{theme_slug}}');
```
...
registerBlockType('{{namespace}}/custom-block', {
    title: __('Custom Block', '{{theme_slug}}'),
    description: __('A custom block for the theme', '{{theme_slug}}'),
    category: 'widgets',
    attributes: {
        content: {
            type: 'string',
            default: __('Default content', '{{theme_slug}}')
        }
    },
    edit: (props) => {
        return (
            <div>
                <p>{__('Edit mode content', '{{theme_slug}}')}</p>
            </div>
        );
    }
});
...
```php
// ❌ NEVER do this
echo __('Hello', '{{theme_slug}}') . ' ' . $name;

// ✅ Do this instead
printf(
    /* translators: %s: User name */
    esc_html__('Hello %s', '{{theme_slug}}'),
    esc_html($name)
);
```
...
echo __($text, '{{theme_slug}}'); // Cannot extract variable content!

// ✅ Do this instead - use conditional
if ($condition) {
    echo esc_html__('Option A', '{{theme_slug}}');
} else {
    echo esc_html__('Option B', '{{theme_slug}}');
}
```
...
echo __('<strong>Bold text</strong> here', '{{theme_slug}}');

// ✅ Do this instead
printf(
    /* translators: %s: Important text */
    esc_html__('%s here', '{{theme_slug}}'),
    '<strong>' . esc_html__('Bold text', '{{theme_slug}}') . '</strong>'
);
```
...
<h1><?php echo __('Title', '{{theme_slug}}'); ?></h1>

// ✅ Correct
<h1><?php echo esc_html__('Title', '{{theme_slug}}'); ?></h1>

// ✅ Even better
<h1><?php esc_html_e('Title', '{{theme_slug}}'); ?></h1>
```
...
- Translator comment patterns in "Translator Comments" show how to annotate placeholders.
- JavaScript translation usage with `@wordpress/i18n` in "JavaScript Translation Functions".
- Template meta information example demonstrates placeholder use with `_n()` and `sprintf`.

## Validation

- Generate assets: `npm run makepot`, `npm run makejson`, `npm run i18n`.
- Lint for i18n issues: PHPCS WordPress i18n sniffs and `@wordpress/eslint-plugin`.
- Spot-check `languages/{{theme_slug}}.pot` for completeness and correct text domain.
...
- [ ] Correct text domain used: `{{theme_slug}}`

...
function lswp_theme_deprecated( $function_name, $version, $replacement = '', $message = '' ) {
	...
	$notice = sprintf(
		...
		__( '%1$s is deprecated since version %2$s.', 'block-theme-scaffold' ),
		$function_name,
		$version
	);
	...
	if ( $replacement ) {
		$notice .= ' ' . sprintf(
			...
			__( 'Use %s instead.', 'block-theme-scaffold' ),
			$replacement
		);
	}
	...
	if ( defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
		...
		error_log( '[block-theme-scaffold Deprecation] ' . $notice );
	}
...
function lswp_theme_deprecated_argument( $function_name, $argument, $version, $message = '' ) {
	...
	if ( function_exists( '_deprecated_argument' ) ) {
		...
		$notice = sprintf(
			...
			__( 'The %1$s argument for %2$s is deprecated since version %3$s.', 'block-theme-scaffold' ),
			$argument,
			$function_name,
			$version
		);
...
function lswp_theme_deprecated_template( $file, $version, $replacement = '', $message = '' ) {
	...
	if ( function_exists( '_deprecated_file' ) ) {
		...
		$notice = sprintf(
			...
			__( 'Template %1$s is deprecated since version %2$s.', 'block-theme-scaffold' ),
			basename( $file ),
			$version
		);
		if ( $replacement ) {

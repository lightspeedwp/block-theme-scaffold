...
if (fs.existsSync(MUSTACHEIGNORE_PATH)) {
	const customIgnores = fs
		.readFileSync(MUSTACHEIGNORE_PATH, 'utf8')
		.split(/\r?\n/)
		.map((line) => line.trim())
...
function inferVariableType(varName) {
	...
	if (n.includes('font') || n.includes('weight') || n.includes('line_height'))
		return 'font';
	...
	if (n.includes('width') || n.includes('size') || n.includes('spacing'))
		return 'number';
	if (n.includes('bool') || n.startsWith('is_') || n.startsWith('has_'))
		return 'boolean';
	return 'string';
}
...
function scanMustacheVariables(options = {}) {
	...
	if (variablesInRegistry.size > 0) {
		undocumented = Array.from(variablesInCode).filter(
			(v) => !variablesInRegistry.has(v)
		);
		unused = Array.from(variablesInRegistry).filter(
			(v) => !variablesInCode.has(v)
		);
	}
...
function displayResults(results, sortedVariables, options = {}) {
	...
	if (results.meta) {
		...
		if (results.meta.unused && results.meta.unused.length > 0) {
			console.log(
				`📋 Unused variables in registry (${results.meta.unused.length}):`
			);
		...
		if (
			results.meta.undocumented.length === 0 &&
			results.meta.unused.length === 0
		) {
	...
	if (categoryEntries.length > 0) {
		...
		categoryEntries.forEach(([category, data]) => {
			console.log(
				`  - ${category}: ${data.count} variable${data.count === 1 ? '' : 's'}`
			);
		});
		console.log('');
	}
	...
	for (let i = 0; i < limit; i += 1) {
		...
		if (showUsage && variable.usage && variable.usage.length > 0) {
			...
			if (variable.usage.length > usageLimit) {
				console.log(
					`     ...and ${variable.usage.length - usageLimit} more locations`
				);
			}
		}
	}
	...
	if (sortedVariables.length > limit) {
		console.log(
			`  ...and ${sortedVariables.length - limit} more variables.`
		);
	}
}
...
function validateConfig(configPath, results) {
	try {
		...
		const discoveredVars = new Set(
			Object.keys(results.variables).map((name) => name.split('|')[0])
		);
		...
		if (missing.length > 0) {
			console.error(
				'Missing theme config keys for discovered placeholders:'
			);
		...
		if (extra.length > 0) {
			console.error(
				'Theme config contains extra keys that are not used in templates:'
			);
...
function main() {
	...
	const sortedVariables = Object.values(variables).sort(
		(a, b) => b.count - a.count
	);
	...
	if (validateIndex !== -1) {
		...
		if (!configPath) {
			console.error(
				'Please provide the path to theme config after --validate'
			);
			process.exit(1);
		}

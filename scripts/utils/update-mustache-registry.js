...
function ensureSchemaStructure(existingSchema) {
	const defaults = {
		...
		description:
			'Auto-generated list of mustache placeholders used in the scaffold',
		type: 'object',
		properties: {},
...
function updateVariablesSchema(names) {
	const schema = {
		...
		description:
			'Enumerates every mustache placeholder currently discovered in the scaffold.',
		type: 'string',
		enum: names,
...
function reportPlaceholderSync(names) {
	...
	const unused = Array.from(placeholderKeys).filter(
...
function main() {
	...
	const { missing, unused } = reportPlaceholderSync(
	...
	if (failOnSync && (missing.length > 0 || registryOutOfSync)) {
		console.error(
			'❌ CI/pre-commit: Registry or placeholder map is out of sync.'
		);
		process.exit(1);
	}
}

...
function validateKnownVariables(schema) {
	...
	if (extra.length > 0) {
		console.log(`ℹ Schema contains ${extra.length} additional variables beyond core set`);
	}
...
function scanCodebaseForMustache() {
	...
	try {
		...
		console.log(`Found ${variables.length} unique mustache variables in codebase`);
...
function validateRegistrySync(schema, codebaseVariables) {
	...
	if (undocumented.length > 0) {
		...
		console.error(`Found ${undocumented.length} undocumented variables:`);
...
function main() {
	...
	console.log('='.repeat(60));
	console.log(`Schema Structure: ${results.structure ? '✓ PASS' : '✗ FAIL'}`);
	console.log(
		`Known Variables:  ${results.knownVariables ? '✓ PASS' : '⚠ WARNING'}`
	);
	console.log(`Registry Sync:    ${results.registrySync ? '✓ PASS' : '✗ FAIL'}`);
	console.log();
	...
	if (exitCode === 0) {
		...
		console.error('1. Add them to .github/schemas/mustache-variables-registry.schema.json');

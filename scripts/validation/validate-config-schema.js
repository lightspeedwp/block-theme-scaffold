...
if (require.main === module) {
	...
	switch (command) {
		...
		default:
			...
			console.log('Usage:');
			console.log(
				'  node scripts/validation/validate-config-schema.js --schema     Output schema as JSON'
			);
			console.log(
				'  node scripts/validation/validate-config-schema.js --stages     List available stages'
			);
			console.log(
				'  node scripts/validation/validate-config-schema.js --keys       List all config keys'
			);
			break;
	}
}

...
/**
 * Load the canonical JSON schema for theme config, with caching.
 * @returns {object|null} Parsed JSON schema object, or null if not found/invalid.
 */
function loadCanonicalJsonSchema() {
	if (_cachedJsonSchema) return _cachedJsonSchema;
	...
	try {
		...
		// Fallback: schema not found or invalid
		return null;
	}
}
...
/**
 * Return the canonical JSON schema (fallback to the JS schema when missing)
 *
 * @returns {Object} The schema object to use for validation/help output
 */
function getCanonicalConfigSchema() {
	const canonicalSchema = loadCanonicalJsonSchema();
	if ( canonicalSchema ) {
...
function validateConfig( config ) {
	...
	const schemaToValidate = getCanonicalConfigSchema();
	for ( const [ key, schema ] of Object.entries( schemaToValidate ) ) {
...
function applyDefaults( config ) {
	...
	const schemaForDefaults = getCanonicalConfigSchema();
	for ( const [ key, schema ] of Object.entries( schemaForDefaults ) ) {
...
function getStageQuestions( stage ) {
	const schemaForQuestions = getCanonicalConfigSchema();
	return Object.entries( schemaForQuestions )

...
function loadCanonicalJsonSchema() {
	...
	const schemaPath = path.resolve( __dirname, '..', '..', '.github', 'schemas', 'theme-config.schema.json' );
	try {
...
const CONFIG_SCHEMA = {
		 theme_slug: {
			...
			minLength: 3,
			description: 'URL-safe theme identifier (lowercase, hyphens only, at least 3 chars, no leading/trailing hyphen, no consecutive hyphens)',
	...
	description: {
		...
		example: 'A modern WordPress block theme built with accessibility and performance in mind.',
		default: 'A modern WordPress block theme built with accessibility and performance in mind.',
	},
	author: {
		stage: 1,
		required: true,
	...
	theme_tags: {
		...
		required: false,
		type: 'string',
		default: 'block-patterns, block-styles, custom-colors, custom-menu, editor-style, featured-images, full-site-editing, template-editing, wide-blocks',
	},
};
...
function applyDefaults( config ) {
       ...
       if ( result.theme_slug && ( result.theme_uri === undefined || result.theme_uri === null || result.theme_uri === '' ) ) {
...
if ( require.main === module ) {
	...
	switch ( command ) {
		...
		case '--stages': {
			const stages = new Set( Object.values( CONFIG_SCHEMA ).map( ( s ) => s.stage ) );
		...
		default:
			...
			console.log( '' );
			console.log( 'Usage:' );
			console.log( '  node scripts/lib/define-config-schema.js --schema     Output schema as JSON' );
			console.log( '  node scripts/lib/define-config-schema.js --stages     List available stages' );
			console.log( '  node scripts/lib/define-config-schema.js --keys       List all config keys' );
			break;
	}
}

...
const wizardInterfaces = {
	...
	cli: async (questions, options = {}) => {
		if (!inquirer)
			throw new Error(
				'inquirer is not installed. Run `npm install inquirer` to use CLI wizard.'
			);
	...
	yaml: async (questions, options = {}) => {
		...
		try {
			...
			throw new Error(
				'js-yaml is not installed. Run `npm install js-yaml` to use YAML wizard.'
			);
		}
		const absPath = path.isAbsolute(configPath)
			? configPath
	...
	http: async (questions, options = {}) => {
		...
		try {
			...
			throw new Error(
				'node-fetch is not installed. Run `npm install node-fetch` to use http wizard.'
			);
		}
		...
		if (contentType.includes('application/json')) {
			...
			try {
				...
				throw new Error(
					'js-yaml is not installed. Run `npm install js-yaml` to parse YAML response.'
				);
			}
				...
				try {
					if (format === 'yaml') {
						...
						try {
							...
							throw new Error(
								'js-yaml is not installed. Run `npm install js-yaml` to parse YAML from stdin.'
							);
						}

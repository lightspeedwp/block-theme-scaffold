/**
 * @file test-dry-run-e2e.js
 * @description Dry-run for E2E tests with mustache placeholder replacement.
 */
const fs = require('fs');
...
const tempDir = path.join(scaffoldDir, '.test-temp-e2e');
...
function main() {
	...
	try {
		execSync('wp-scripts test-e2e', { cwd: tempDir, stdio: 'inherit' });
	} catch (error) {
		process.exit(1);
	}
	cleanup();
